# Implementation of  precision and recall for evaluating generative models.
# Original code was taken from
# https://github.com/kynkaat/improved-precision-and-recall-metric

# Modified by Y. Yamaguti in 2023 for use with TensorFlow 2
# Simplified to remove multi-GPU support
        ...
        for begin1 in range(0, num_images, row_batch_size):
            ...
            for begin2 in range(0, num_images, col_batch_size):
                ...
                distance_batch[0:end1-begin1,
                               begin2:end2] = batch_pairwise_distances(row_batch, col_batch)
    ...
    def evaluate(self, eval_features, return_realism=False, return_neighbors=False):
        ...
        for begin1 in range(0, num_eval_images, self.row_batch_size):
            ...
            for begin2 in range(0, num_ref_images, self.col_batch_size):
                ...
                distance_batch[0:end1-begin1, begin2:end2] = batch_pairwise_distances(
...
def knn_precision_recall_features(ref_features, eval_features, nhood_sizes=[3],
                                  row_batch_size=2000, col_batch_size=50000, num_gpus=1):
    ...
    state['precision'] = precision.mean(axis=0)  # type:ignore

    # Recall: How many points from ref_features are in eval_features manifold.
    recall = eval_manifold.evaluate(ref_features)
    state['recall'] = recall.mean(axis=0)  # type:ignore
...
def get_pretrained_classifier_model(headless=True):
  '''
  returns pretrained classifier model (VGG16, currently)
  '''
  model = VGG16(
      include_top=True,
      weights='imagenet',)
...
def convert_mnist_to_imagenet_size(mnist_image, min_val=-1, max_val=1, image_size=(224, 224)):
  '''
  convert mnist image to ImageNet-size image.
  It also set range of the values to [0,255].
  '''
  # resize mnist image to ImageNet-size image
  mnist_image = 255*(mnist_image-min_val) / \
      (max_val-min_val)  # normalize to [0,255] range
  imagenet_image = tf.image.resize(mnist_image, image_size)
  # gray-scale to color image
  imagenet_image = tf.image.grayscale_to_rgb(imagenet_image)

  return imagenet_image
