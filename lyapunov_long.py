...
# より長い軌道を1バッチとってリアプノフ指数を計算する．収束をみる．
# %%
...
def iterate_g(x0, t_max=10, g=generator_g, transient=0, stack=True) -> tf.Tensor:
    ...
    for t in range(transient, t_max):
        # print(f't={t}')
        x_new = g_func(x)
        X.append(x_new.numpy())  #
        x = x_new

    # if stack:
    # print(X[0].shape)
    # print(len(X))
    X = np.stack(X, axis=1)  # (bs, len_seq, h,w,c)
    return X
...
# %%
def trajectories_from_x0s(iteration, x0, t_max, transient=0):
    ...
    Done for each mini-batch.
        ...
        x0: the initial images (bs, h,w,c) data
    ...
    # Zs = []
    print(x0.shape)
    X = iteration(x0, t_max=t_max, transient=transient)
    # X = X.numpy()

    nh = X.shape[2]
    nw = X.shape[3]
    nc = X.shape[4]
    X = X.reshape((-1, t_len, nh*nw*nc))
    return X


# %%
# Xs, Ys, Zs = trajectories_from_ds(iterate_g, ds_test_x, t_max=t_max, transient=transient)
x0 = next(iter(ds_test_x))
Xs = trajectories_from_x0s(iterate_g, x0,
                          t_max=t_max, transient=transient)

print(Xs.shape)  # (bs, t_len, 784)
...
np.save(os.path.join(data_dir, 'Xs_long.npy'), Xs)

# %%

Xs = np.load( os.path.join(data_dir, 'Xs_long.npy'))
...
for ii, xb in enumerate(X_split):
    ...
    X_t = [xb[:,i,:] for i in range(xb.shape[1])]  # each element of list have (bs, 784)
    ...
    logrs = np.stack(logrs, axis=1)  # (bs, len, 784)
    li_logrs.append(logrs)


logrs = np.concatenate(li_logrs, axis=0)
lsps = np.mean(logrs, axis=1)  # (bs,784)


# %% 過程も保存
np.save(os.path.join(data_dir, 'logrs_long.npy'), logrs)#(bs, tlen, 784) # type:ignore
np.save(os.path.join(data_dir, 'lsps_long.npy'), lsps)

# %% save text file for logrs
with open(os.path.join(data_dir, 'lsps_long.txt'), 'w') as f:
    for l in lsps:
