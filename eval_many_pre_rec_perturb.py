...
file_path = os.path.join(trj_many_data_dir, 'Xs_used_perturb.npy')
Xs_x = np.load(file_path)
file_path = os.path.join(trj_many_data_dir, 'Xs_used_y_perturb.npy')
Xs_y = np.load(file_path)
file_path = os.path.join(trj_many_data_dir, 'Xs_used_z_perturb.npy')
...
print(Xs.shape)  # (3197*5, len(steps_used), 28, 28, 1)
# assert Xs.shape[1] == len(steps_used)
# assert Xs.shape[0] == n_test*5

#%% perturbed test image dataset and train image dataset

Xtest_p = test_images_xyz = Xs[:,0,:,:,:].reshape(-1,28,28,1)

train_images_xyz = np.concatenate(
                    (train_images_x[0:Xs_x.shape[0]],
                    train_images_y[0:Xs_y.shape[0]],
                    train_images_z[0:Xs_z.shape[0]]), axis=0)
...
phi_gen = []
for t in tqdm(range(Xs.shape[1])):
...
# save
plt.savefig(os.path.join(fig_dir, 'feature_pca_test_train_perturb.png'))
...
fname = 'feature_umap_test_train_perturb'
...
for t in range(phi_gen_umap.shape[0]):
  ...
  fname = f'feature_umap_test_gen_many_step{step}_perturb'
...
ax.legend()
fig.tight_layout()
fname = 'precision_recall_vs_steps_perturb'
fig.savefig(os.path.join(fig_dir, fname+'.png'), dpi=360)
fig.savefig(os.path.join(fig_dir, fname+'.tiff'), dpi=360)
fig.savefig(os.path.join(fig_dir, fname+'.pdf'), )
